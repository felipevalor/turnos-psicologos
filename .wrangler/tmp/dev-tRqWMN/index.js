...
// .wrangler/tmp/bundle-caklWL/checked-fetch.js
...
slotsRouter.get("/", async (c) => {
  ...
  const availableSlots = existingSlotsResult.results.filter((s) => {
    if (Number(s.disponible) !== 1)
      return false;
    if (s.fecha < cutoffDateStr)
      return false;
    if (s.fecha === cutoffDateStr && s.hora_inicio <= cutoffTimeStr)
      return false;
    return true;
        ...
        try {
          await db.prepare(
            `INSERT INTO reservas (slot_id, paciente_nombre, paciente_email, paciente_telefono)
               VALUES (?, ?, ?, ?)`
          ).bind(existingSlot.id, patientName, patientEmail, patientPhone).run();
...
recurringRouter.get("/", authMiddleware, async (c) => {
  ...
  try {
    ...
    ).bind(todayStr(), psychologistId).all();
    return c.json({ success: true, data: result.results });
  } catch (error) {
...
recurringRouter.delete("/:id", async (c) => {
  ...
  try {
    ...
    const isTotalCancellation = !from_date || !isValidDate2(from_date);
...
// .wrangler/tmp/bundle-caklWL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-caklWL/middleware-loader.entry.ts
