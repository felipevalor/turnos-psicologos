...
// .wrangler/tmp/bundle-X4Pdny/checked-fetch.js
...
var authMiddleware = createMiddleware(async (c, next) => {
  ...
  if (cookieHeader) {
    const match2 = cookieHeader.match(/(?:^|;\s*)psi_session=([^;]+)/);
...
async function isRateLimited(env, ip) {
  ...
  const now = Date.now();
  if (env.CACHE) {
    const raw2 = await env.CACHE.get(key);
    if (!raw2) return false;
    const entry2 = JSON.parse(raw2);
    if (now - entry2.windowStart > RATE_WINDOW_MS) return false;
    return entry2.count >= MAX_ATTEMPTS;
  }
...
async function recordFailedAttempt(env, ip) {
  ...
  if (env.CACHE) {
    const raw2 = await env.CACHE.get(key);
    const entry2 = raw2 ? JSON.parse(raw2) : { count: 0, windowStart: now };
    if (now - entry2.windowStart > RATE_WINDOW_MS) {
      entry2.count = 0;
      entry2.windowStart = now;
    }
    entry2.count++;
    await env.CACHE.put(key, JSON.stringify(entry2), { expirationTtl: RATE_WINDOW_MS / 1e3 });
    return;
  }
...
authRouter.patch("/me", authMiddleware, async (c) => {
  ...
  if (whatsapp_number !== void 0) {
    if (whatsapp_number !== null && !/^\+\d{7,15}$/.test(whatsapp_number)) {
      return c.json({ success: false, error: "Formato de WhatsApp inv\xE1lido. Use formato internacional: +549xxxxxxxxxx" }, 400);
    }
  ...
  if (updates.length > 0) {
    ...
    await c.env.DB.prepare(`UPDATE psicologos SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
  }
  const psych = await c.env.DB.prepare(
    `SELECT id, nombre as name, email, session_duration_minutes,
            cancel_min_hours, reschedule_min_hours, booking_min_hours, whatsapp_number, policy_unit
...
authRouter.post("/login", async (c) => {
  ...
  if (await isRateLimited(c.env, ip)) {
    return c.json(
      { success: false, error: "Demasiados intentos fallidos. Esper\xE1 15 minutos." },
      429
    );
  }
  let body;
...
authRouter.post("/logout", (c) => {
  const isSecure = new URL(c.req.url).protocol === "https:";
  const cookie = isSecure ? "psi_session=; HttpOnly; Secure; SameSite=None; Path=/; Max-Age=0" : "psi_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0";
...
var KV_CACHE_TTL_SECONDS = 86400;
async function fetchArgentineHolidays(year, kv) {
...
// worker/src/lib/date.ts
function getLocalISODate(date) {
  const utcMs = date.getTime();
  const baMs = utcMs - 3 * 3600 * 1e3;
  return new Date(baMs).toISOString().split("T")[0];
}
__name(getLocalISODate, "getLocalISODate");
function getTodayDateString() {
  return getLocalISODate(/* @__PURE__ */ new Date());
}
...
async function generateSlotsForDate(db, date, psychologistId, kv) {
...
slotsRouter.get("/", async (c) => {
  ...
  const BA_OFFSET_MS = -3 * 60 * 60 * 1e3;
  ...
  const cutoffUtcMs = nowUtcMs + thresholdMinutes * 60 * 1e3;
  ...
  void nowBaMs;
  const availableSlots = existingSlotsResult.results.filter((s) => {
...
bookingsRouter.get("/", authMiddleware, async (c) => {
  ...
  const result = await c.env.DB.prepare(
    `SELECT b.id, b.paciente_nombre as patient_name, b.paciente_email as patient_email, b.paciente_telefono as patient_phone, b.created_at,
     ...
     LIMIT ? OFFSET ?`
  ).bind(psychologistId, limit, offset).all();
  return c.json({ success: true, data: result.results });
});
bookingsRouter.post("/", async (c) => {
  ...
  if (patient_name.length > 100) {
    return c.json({ success: false, error: "Nombre demasiado largo (m\xE1ximo 100 caracteres)" }, 400);
  }
  if (patient_email.length > 254 || !patient_email.includes("@")) {
    return c.json({ success: false, error: "Email inv\xE1lido" }, 400);
  }
  ...
  try {
    ...
    const result = await c.env.DB.prepare(
      `SELECT b.id, b.paciente_nombre as patient_name, b.paciente_email as patient_email, b.paciente_telefono as patient_phone, b.created_at,
       ...
       AND s.fecha >= ?
       ORDER BY s.fecha, s.hora_inicio`
    ).bind(...params, getTodayDateString()).all();
    return c.json({ success: true, data: result.results });
  } catch (error) {
...
bookingsRouter.patch("/:id", async (c) => {
  ...
  if (!new_slot_id) {
    return c.json({ success: false, error: "Seleccion\xE1 un nuevo turno" }, 400);
  }
  ...
  try {
    const results = await c.env.DB.batch([
      ...
      c.env.DB.prepare(
        ...
        oldBooking.psicologo_id,
        oldBooking.slot_id,
        oldBooking.fecha,
        oldBooking.hora_inicio,
        oldBooking.paciente_nombre,
        oldBooking.paciente_email,
        oldBooking.paciente_telefono
      ),
      // Free old slot
...
bookingsRouter.delete("/:id", async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      ...
      booking.psicologo_id,
      booking.slot_id,
      booking.fecha,
      booking.hora_inicio,
      booking.paciente_nombre ?? "",
      booking.paciente_email,
...
function matchesRecurrence(fecha, startDate, frequencyWeeks) {
  if (fecha < startDate) return false;
  const d1 = /* @__PURE__ */ new Date(`${startDate}T12:00:00Z`);
  const d2 = /* @__PURE__ */ new Date(`${fecha}T12:00:00Z`);
  const diffTime = d2.getTime() - d1.getTime();
  const diffDays = Math.round(diffTime / (1e3 * 60 * 60 * 24));
  return diffDays % (frequencyWeeks * 7) === 0;
}
__name(matchesRecurrence, "matchesRecurrence");
async function generateSlots(db, params) {
  ...
  while (current <= toDate) {
    ...
    ).bind(psychologistId, current, time).first();
    if (existingSlot) {
...
recurringRouter.post("/", authMiddleware, async (c) => {
  ...
  if (!patient_name || !patient_email || !patient_phone || !start_date || !time || !frequency_weeks) {
    return c.json(
      {
        success: false,
        error: "Campos requeridos: nombre del paciente, email, tel\xE9fono, fecha de inicio, hora, frecuencia"
      },
      400
    );
  }
  ...
  const d = /* @__PURE__ */ new Date(`${start_date}T12:00:00Z`);
  const dayOfWeek = d.getUTCDay();
  const scheduleWindow = await c.env.DB.prepare(
    "SELECT active FROM weekly_schedule WHERE psychologist_id = ? AND day_of_week = ?"
  ).bind(psychologistId, dayOfWeek).first();
  if (!scheduleWindow || scheduleWindow.active === 0) {
    return c.json({ success: false, error: "La fecha de inicio cae en un d\xEDa no laboral seg\xFAn tu agenda semanal" }, 400);
  }
...
recurringRouter.get("/", authMiddleware, async (c) => {
  ...
  try {
    ...
    ).bind(getTodayDateString(), psychologistId).all();
    return c.json({ success: true, data: result.results });
  } catch (error) {
  ...
  try {
    ...
    const orphanQuery = `SELECT s.id, s.fecha FROM slots s
       ...
       WHERE s.psicologo_id = ?
         ...
         AND s.fecha < ?
         AND r.id IS NULL`;
    const bookedCandidates = await c.env.DB.prepare(bookedQuery).bind(recurring.psychologist_id, recurring.time, targetDate, recurring.patient_email, recurring.patient_phone).all();
    const bookedSlotIds = bookedCandidates.results.filter((s) => matchesRecurrence(s.fecha, recurring.start_date, recurring.frequency_weeks)).map((s) => s.id);
    let orphanSlotIds = [];
    if (isTotalCancellation) {
      const orphanCandidates = await c.env.DB.prepare(orphanQuery).bind(recurring.psychologist_id, recurring.time, targetDate).all();
      orphanSlotIds = orphanCandidates.results.filter((s) => matchesRecurrence(s.fecha, recurring.start_date, recurring.frequency_weeks)).map((s) => s.id);
    }
    const batchSize = 50;
...
recurringRouter.patch("/:id", authMiddleware, async (c) => {
  ...
  const res = await c.env.DB.prepare("UPDATE recurring_bookings SET frequency_weeks = ? WHERE id = ? AND psychologist_id = ? AND active = 1").bind(frequency_weeks, id, psychologistId).run();
...
recurringRouter.patch("/:id/reschedule-from", async (c) => {
  ...
  const candidateSlots = futureSlots.results.filter(
    (s) => matchesRecurrence(s.date, recurring.start_date, recurring.frequency_weeks)
  );
...
__name(getBaDate, "getBaDate");
dashboardRouter.get("/", authMiddleware, async (c) => {
  const psychologistId = c.get("psychologistId");
  const baDate = getBaDate();
  const [
    todaySessions,
    // Week queries
    weekTotal,
...
notesRouter.post("/", async (c) => {
  ...
  if (!patient_email || !contenido) {
    return c.json({ success: false, error: "Email y contenido s\xE3o requeridos" }, 400);
  }
  const result = await c.env.DB.prepare(
    `INSERT INTO paciente_notas (psicologo_id, paciente_email, contenido, slot_id)
     VALUES (?, ?, ?, ?) RETURNING id, created_at, updated_at`
  ).bind(psychologistId, patient_email, contenido, slot_id || null).first();
...
notesRouter.put("/:id", async (c) => {
  ...
  ).bind(contenido, id, psychologistId).run();
...
// worker/src/routes/patients.ts
var patientsRouter = new Hono2();
...
patientsRouter.get("/", async (c) => {
  ...
  `;
  const result = await c.env.DB.prepare(query).bind(psychologistId, psychologistId, psychologistId, psychologistId, psychologistId, psychologistId).all();
  return c.json({ success: true, data: result.results });
});
...
// .wrangler/tmp/bundle-X4Pdny/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-X4Pdny/middleware-loader.entry.ts
